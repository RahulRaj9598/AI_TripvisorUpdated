...
import { authenticateUser, optionalAuth, generateJWTToken } from '../middleware/auth.js';
...
// Update user profile
router.put('/profile', authenticateUser, uploadSingle, handleUploadError, async (req, res) => {
  try {
...
router.get('/:userId/followers/mutual', authenticateUser, async (req, res) => {
  try {
    const { page = 1, limit = 20 } = req.query;
...
router.get('/:userId/following/mutual', authenticateUser, async (req, res) => {
  try {
    const { page = 1, limit = 20 } = req.query;
...
router.get('/:userId/mutual-status', authenticateUser, async (req, res) => {
  try {
    const targetUser = await User.findById(req.params.userId);
    const currentUser = await User.findById(req.user._id);
...
router.get('/favorites/blogs', authenticateUser, async (req, res) => {
  try {
    const { page = 1, limit = 10 } = req.query;
...
router.post('/favorites/blogs/:blogId', authenticateUser, async (req, res) => {
  try {
